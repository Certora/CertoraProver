...
class CloudVerification:
    ...
    def prepare_auth_data(self, cl_args: str) -> Optional[Dict[str, Any]]:
        ...
        if Attrs.is_rust_app():
            ...
            if self.context.sui_package_summary_path and self.context.sui_package_summary_path.exists():
                sui_jar_settings.append('-suiPackageSummaryPath')
                sui_jar_settings.append(self.context.sui_package_summary_path.name)
    ...
    def __compress_and_upload_zip_files(self) -> bool:
        ...
        if self.context.is_tac:
            ...
            if self.context.sui_package_summary_path and self.context.sui_package_summary_path.exists():
                files_list.append(Util.get_build_dir() / self.context.sui_package_summary_path.name)
