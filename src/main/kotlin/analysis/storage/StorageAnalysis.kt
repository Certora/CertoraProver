...
class StorageAnalysis(private val compilerStorage: TACStorageLayout?, private val contractClass: IContractClass?, private val base: Base) {
    ...
    enum class Base(val prefixChar: String, val storageKey: MetaKey<BigInteger>, val isStorageAccessKey: MetaKey<Nothing>, val statisticsKey: String) {
        STORAGE("S", TACMeta.STORAGE_KEY, TACMeta.IS_STORAGE_ACCESS, ANALYSIS_STORAGE_SUBKEY),
        TRANSIENT_STORAGE("T", TACMeta.TRANSIENT_STORAGE_KEY, TACMeta.IS_TRANSIENT_STORAGE_ACCESS, ANALYSIS_TRANSIENT_STORAGE_SUBKEY);
        ...
        sealed class StoragePointer : SValue(), Indexable {
            ...
            data class FieldPointer(override val base: Set<Storage>, val offset: BigInteger, val accessPaths: AnalysisPaths) : StoragePointer() {
                ...
                override fun widen(
                    ...
                    /**
                     * This special rule detects a case where `this` (aka `prev` in widening) points to the
                     * beginning of a static array A (in some mapping, struct, or array) and [j] (aka `next`) points to the beginning
                     * of some element in A. From this, we extrapolate that the abstract value is meant to represent "some element
                     * of the static array", given that "the start of the static array" is also the same thing as "element 0 of the
                     * static array". We do this by taking [j] as our abstract value, but nulling out the index used in the
                     * static array access path.
                     */
    ...
    private inner class MethodWorker(val graph: TACCommandGraph, val linearInvariants: GlobalInvariantAnalysisResult) {
        ...
        private fun handleFieldAddition(context: CmdPointer, v: Set<Storage>, offset: BigInteger, parentPath: AnalysisPaths) : Pair<SValue, Boolean> {
            ...
            val staticArrayElemSize = v.monadicMap { ref ->
            ...
            } else if(staticArrayElemSize == offset) {
