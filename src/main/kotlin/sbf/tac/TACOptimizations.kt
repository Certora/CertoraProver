...
fun solanaOptimize(coreTAC: CoreTACProgram, rule: EcosystemAgnosticRule): CoreTACProgram {
    val optCoreTAC = timeIt("[${rule.ruleIdentifier}]", "TAC optimizations") {
        val levelZeroOptCoreTac = levelZeroOptimizations(coreTAC, rule.isSatisfyRule)
        if (SolanaConfig.UseLegacyTACOpt.get()) {
            legacyOptimize(levelZeroOptCoreTac)
        } else {
            optimize(levelZeroOptCoreTac)
        }
    }
...
fun optimize(coreTAC: CoreTACProgram): CoreTACProgram {
...
/**
 * First runs DSA and then unrolls loops.
 */
...
fun levelZeroOptimizations(coreTAC: CoreTACProgram, isSatisfyRule: Boolean): CoreTACProgram =
    CoreTACProgram.Linear(coreTAC)
        .let { CoreTACProgram.Linear(runDSAandUnrollLoops(it.ref)) }
        .also {
